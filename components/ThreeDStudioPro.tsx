...
const ThreeDStudioPro: React.FC = () => {
    ...
    }, [characterCount, characters, synopsis, sceneCount, scenes]);
    ...
    const handleRenderAll = async () => {
        ...
        try {
            ...
            for (let i = 0; i < scenesToRender.length; i += RENDER_CHUNK_SIZE) {
                ...
                await Promise.all(chunk.map(async (scene) => {
                    if (!scene.promptUsed) return;
                    setScenes(prev => prev.map(s => s.sceneNumber === scene.sceneNumber ? { ...s, isLoading: true } : s));
                    try {
                        const imageUrl = await generateImage(scene.promptUsed, '16:9');
                        setScenes(prev => prev.map(s => s.sceneNumber === scene.sceneNumber ? { ...s, imageUrl: imageUrl, isLoading: false } : s));
                    } catch (err) {
    ...
    const handleRegenerateSenseImage = async (index: number) => {
        ...
        setScenes(prev => prev.map((s, i) => i === index ? { ...s, isLoadingImage: true, imageUrl: undefined } : s));

        try {
            const imageUrl = await generateImage(scene.promptUsed, '16:9');
            setScenes(prev => prev.map((s, i) => i === index ? { ...s, imageUrl: imageUrl, isLoadingImage: false } : s));
        } catch (err) {
    ...
    const handleRegeneratePromptWithExtension = async (idx: number) => {
        ...
        try {
            ...
            if (result.action) {
                ...
                const newImg = await generateImage(fullPrompt, '16:9');
                setScenes(prev => prev.map(s => s.sceneNumber === scene.sceneNumber ? { ...s, imageUrl: newImg, isLoading: false } : s));
                                    ...
                                    <div key={char.id} className="p-3 bg-[#252b3d] rounded-xl border border-gray-700/50 shadow-inner flex flex-col gap-3 group">
                                        <div className="flex gap-4">
                                            <div className="w-16 h-16 bg-[#0f172a] rounded-lg border-2 border-dashed border-gray-600 flex items-center justify-center shrink-0 overflow-hidden relative">
                                                {char.image ? <img src={`data:${char.image.mimeType};base64,${char.image.base64}`} className="w-full h-full object-cover" /> : <label className="cursor-pointer text-[10px] text-gray-500 font-bold uppercase p-2 text-center leading-tight">📷 Upload Model</label>}
                        ...
                        {scenes.map((scene, idx) => (
                            <div key={idx} className="bg-[#1a1f2e] rounded-2xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col group hover:border-purple-500/50 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? <img src={scene.imageUrl} className="w-full h-full object-cover" alt={`Sense ${scene.sceneNumber}`} /> : <div className="flex flex-col items-center gap-3">{scene.isLoading ? <><Spinner className="h-10 w-10 text-purple-500" /><span className="text-[10px] text-gray-500 font-black uppercase animate-pulse">Rendering 3D...</span></> : <span className="text-[10px] text-gray-700 font-black uppercase">Ready to Render</span>}</div>}
                                ...
                                <div className="p-4 flex-grow flex flex-col bg-gradient-to-b from-[#1a1f2e] to-[#0f172a]">
                                    ...
                                    {editingSceneIndex === idx && (
                                        <div className="bg-indigo-900/20 border border-indigo-500/30 p-4 rounded-xl mb-4 animate-fade-in">
