...
import { 
    ...
    generateImage, 
    generateRoomCleaningIdeas, // Reuse similar concept for spins box
    CarIdea, 
...
const FarmingProGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [masterPrompt, setMasterPrompt] = useState('Modern Automatic Farming: High-tech autonomous machinery plowing, planting, and harvesting giant vegetable fields. កសិកម្មទំនើបប្រើម៉ាស៊ីនស្វ័យប្រវត្តិ។ Focus on satisfying industrial efficiency, soil textures, and advanced robotic tools.');
    const [sceneCount, setSceneCount] = useState(15);
    const [scenes, setScenes] = useState<Scene[]>([]);
    const [isGeneratingScript, setIsGeneratingScript] = useState(false);
    const [isGeneratingIdeas, setIsGeneratingIdeas] = useState(false);
    const [farmIdeas, setFarmIdeas] = useState<CarIdea[]>([]);
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            if (e.detail.tool !== 'farming-pro') return;
            const data = {
                id: crypto.randomUUID(),
                timestamp: Date.now(),
                tool: 'farming-pro',
                category: 'vip',
                title: "Modern Farming Project",
    ...
    const handleGenerateIdeas = async () => {
        ...
        setError(null);
        try {
            // Reusing logic but targeting farming theme
            const ideas = await generateRoomCleaningIdeas("Amazing Automatic Farming Machines: " + masterPrompt);
            setFarmIdeas(ideas);
        } catch (err) {
            setError("Failed to generate farm ideas.");
        } finally {
    ...
    const handleUseIdea = (idea: CarIdea) => {
        setMasterPrompt(`Farming Concept: ${idea.title}\n\nVision: ${idea.description}\n\nFocus on the satisfying ASMR sounds of modern agriculture: soil turning, seeds dropping, mechanical arms moving, and the 100% efficient harvest process.`);
        setFarmIdeas([]);
    };
    ...
    const handleGenerateScript = async () => {
        ...
        const voiceInstruction = noVoiceover ? "\n\nSTRICT RULE: Do NOT include any spoken dialogue or narration. Focus purely on industrial farming sounds: mechanical hums, soil crunching, hydraulic hisses." : "";

        try {
            const result = await generateConsistentStoryScript(
                `MODERN AUTOMATIC FARMING PRODUCTION. Context: ${masterPrompt}. Style: 100% Realistic, Professional Agricultural & Industrial Photography. Show the satisfying efficiency from plowing to shipping containers.${voiceInstruction}`,
                sceneCount
            );
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic, professional industrial agriculture photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Detailed textures of fertile soil, stainless steel machinery, fresh green crops, cinematic sunlight, wide angle. 100% consistent machines.`;
    ...
    const handleCopyJson = (index: number) => {
        ...
        const structuredData = {
            ...
            prompt: `100% Realistic, professional industrial agriculture photography, 8k. Action: ${scene.action}. Context: ${scene.consistentContext}.`
        };
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `Farming_Sense_${num}.png`;
    ...
    const handleClear = () => {
        setMasterPrompt('Modern Automatic Farming: High-tech autonomous machinery plowing, planting, and harvesting giant vegetable fields. កសិកម្មទំនើបប្រើម៉ាស៊ីនស្វ័យប្រវត្តិ។ Focus on satisfying industrial efficiency, soil textures, and advanced robotic tools.');
        setScenes([]);
        setFarmIdeas([]);
        setNoVoiceover(true);
        setYoutubeMeta(null);
    ...
    const handleGenerateMetadata = async () => {
        ...
        setError(null);
        try {
            const context = `Modern Farming Project: ${masterPrompt}\n\nStoryboard:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(
                "Amazing Automatic Farming Machines | Modern Agriculture Efficiency",
                context,
                "Technology & Farming"
            );
            setYoutubeMeta(meta);
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in">
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-green-600 mb-2 flex items-center gap-2">
                        <span>🌾🚜</span> Farming Pro
                    </h2>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest border-b border-gray-700 pb-4">
                        Modern Agriculture Storyboard.
                    </p>
                    ...
                    <div className="space-y-4">
                        ...
                        {farmIdeas.length > 0 && (
                            <div className="space-y-2 animate-slide-down bg-black/40 p-3 rounded-xl border border-gray-700 mb-4">
                                <h4 className="text-[10px] font-black text-emerald-500 uppercase tracking-tighter mb-2">Select a Machine Theme:</h4>
                                {farmIdeas.map((idea, idx) => (
                        ...
                        <textarea 
                            ...
                            placeholder="Describe the farming technology flow..."
                    ...
                    <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700">
                        <label className="flex items-center gap-3 cursor-pointer group">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-[9px] text-gray-500 italic">Focus on mechanical sounds.</span>
                            </div>
                        </label>
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full py-4 bg-gradient-to-r from-emerald-600 to-green-600 hover:brightness-110 text-white font-black rounded-xl shadow-xl transition transform active:scale-[0.98] disabled:opacity-50 text-lg flex items-center justify-center gap-3 uppercase tracking-widest"
                    >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[85vh] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        ...
                        {scenes.length === 0 && !isGeneratingScript && (
                            <div className="col-span-full py-20 text-center bg-gray-900/20 rounded-[3rem] border-4 border-dashed border-gray-800 flex flex-col items-center justify-center">
                                 ...
                                 <p className="text-xl font-black text-gray-600 uppercase tracking-[0.4em]">Agricultural Floor Ready</p>
                                 <p className="text-sm text-gray-700 mt-4 max-w-md">Enter your scenario or use AI ideas to generate modern farming production senses.</p>
                            </div>
                        )}
                    </div>
                </div>
