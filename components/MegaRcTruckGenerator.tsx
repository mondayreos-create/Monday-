...
const MegaRcTruckGenerator: React.FC = () => {
    ...
    const handleReloadHistory = (project: any) => {
        if (project.data.vehicleImages) setVehicleImages(project.data.vehicleImages);
        ...
        if (project.data.youtubeMeta) setYoutubeMeta(project.data.youtubeMeta);
    ...
    // FIX: Added downloadRefImage function to fix the "Cannot find name 'downloadRefImage'" error.
    ...
    const handleCopyAllSenses = () => {
        ...
        // Correcting per request: copy promptUsed for video generation
    ...
    const handleCopyAllJson = () => {
        if (scenes.length === 0) return;
        // Correcting per request: copy specific code prompt
                        ...
                        {localHistory.length > 0 ? (
                            localHistory.map((project, idx) => (
                                ...
                                >
                                    ...
                                    <p className="text-white text-xs font-bold truncate mb-1">{project.data.topic || "Untitled Project"}</p>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-2 mb-2">
                                ...
                                {vehicleImages.length < 5 && (
                                    <label className="cursor-pointer aspect-square bg-gray-900 rounded-lg border-2 border-dashed border-gray-600 flex flex-col items-center justify-center hover:border-orange-400 transition-colors">
                        ...
                        {activeResultTab === 'storyboard' && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {scenes.map((scene, idx) => (
                                    <div key={idx} className="bg-gray-900 rounded-xl overflow-hidden border border-gray-800 flex flex-col hover:border-orange-500/50 transition-all group shadow-xl">
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div className="bg-black/30 p-4 rounded-xl border border-gray-700">
                                                    <div className="flex justify-between items-center mb-2 text-[9px] font-black text-gray-500 uppercase tracking-widest">
                                                        <span>Hashtags</span>
                                                        <button onClick={() => handleCopy(youtubeMeta.hashtags.join(' '), 'metaTags')} className="text-cyan-500 hover:text-cyan-400 transition-colors uppercase font-bold">{copyStatus === 'metaTags' ? 'Copied!' : 'Copy'}</button>
                                                    </div>
                                                    <div className="text-blue-400 font-bold text-[10px]">{youtubeMeta.hashtags.join(' ')}</div>
                                                </div>
                                                <div className="bg-black/30 p-4 rounded-xl border border-gray-700">
                                                    <div className="flex justify-between items-center mb-2 text-[9px] font-black text-gray-500 uppercase tracking-widest">
                                                        <span>Keywords</span>
                                                        <button onClick={() => handleCopy(youtubeMeta.keywords.join(', '), 'metaKeys')} className="text-cyan-500 hover:text-cyan-400 transition-colors uppercase font-bold">{copyStatus === 'metaKeys' ? 'Copied!' : 'Copy'}</button>
                                                    </div>
