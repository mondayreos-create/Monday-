...
const Animals4DGenerator: React.FC = () => {
    ...
    const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            if (characterImages.length >= 5) {
                setError("Maximum 5 characters allowed.");
                return;
            }
    ...
    const handleAutoSetup = async () => {
        ...
        try {
            ...
            const prompt = `Create ${count} distinct distinct cute 3D animal characters for this story: "${synopsis}". Include their names, types (e.g., Elephant, Monkey), and highly detailed visual characteristics.`;
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            // PHASE 2: HIGH-SPEED BATCH SCRIPTING (PROMPTS FIRST)
            ...
            for (let b = 0; b < numBatches; b++) {
                ...
                // Scripting phase accounts for first 30% of total progress
            ...
            // PHASE 3: AUTO GENERATE IMAGES
            setStatusText('Auto-Generating Images for All Senses...');
            const CONCURRENT_RENDERS = 4; // Render in chunks of 4 for speed
            for (let i = 0; i < initialScenes.length; i += CONCURRENT_RENDERS) {
                ...
                // Progress update for rendering (30% to 100%)
                setProgress(30 + Math.round(((i + chunk.length) / initialScenes.length) * 70));
                
                // Small delay between chunks to avoid hard API rate limits while staying fast
    ...
    const handleCopySenseData = (index: number, mode: 'json' | 'prompt') => {
        ...
        /* FIX: Corrected variable name from setCopyStatus to setCopyState. */
        setTimeout(() => setCopyState(null), 2000);
    };
                    ...
                    <div className="space-y-6">
                        <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <button onClick={handleShufflePresets} className="text-[10px] text-gray-500 hover:text-white flex items-center gap-1 transition"><RefreshIcon /> New Contents</button>
                            </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-2 mb-2">
                                ...
                                {characterImages.length < 5 && (
                                    <label className="cursor-pointer aspect-square bg-gray-900 rounded-lg border-2 border-dashed border-gray-600 flex flex-col items-center justify-center hover:border-green-400 transition-colors">
                                ...
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            {scene.isLoading ? <><Spinner className="h-10 w-10 text-green-500" /><span className="text-[10px] text-gray-500 font-black uppercase animate-pulse">Rendering 3D Art...</span></> : <span className="text-[10px] text-gray-700 font-black">READY</span>}
                                        </div>
                                    )}
