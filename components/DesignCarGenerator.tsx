...
const LockIcon: React.FC<{className?: string}> = ({className = "h-3 w-3"}) => (
...
const DesignCarGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [masterPrompt, setMasterPrompt] = useState('Design Car (ASMR): Restoring a classic vintage car from old and rusty to 100% brand new. រថយន្តចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើង វិញ។ Focus on satisfying high-detail ASMR steps like washing, sanding, replacing broken parts with new ones, and repainting the original color to high-gloss perfection.');
    ...
    const [repaintWhite, setRepaintWhite] = useState(false);
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            if (e.detail.tool !== 'design-car') return;
            const data = {
                ...
                data: { masterPrompt, sceneCount, scenes, carIdeas, noVoiceover, repaintWhite, youtubeMeta }
            };
        ...
        const handleLoad = (e: any) => {
            ...
            if (d.carIdeas) setCarIdeas(d.carIdeas);
            if (d.noVoiceover !== undefined) setNoVoiceover(d.noVoiceover);
            if (d.repaintWhite !== undefined) setRepaintWhite(d.repaintWhite);
    ...
    }, [masterPrompt, sceneCount, scenes, carIdeas, noVoiceover, repaintWhite, youtubeMeta]);
    ...
    const loadLocalHistory = () => {
        ...
        if (historyRaw) {
            try {
                ...
                const toolHistory = history.filter((p: any) => p.tool === 'design-car');
    ...
    const handleGenerateIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateCarRestorationIdeas("Viral Car Restoration ASMR: " + masterPrompt);
            setCarIdeas(ideas);
        } catch (err) {
            setError("Failed to generate car ideas.");
        } finally {
    ...
    const handleUseIdea = (idea: CarIdea) => {
        setMasterPrompt(`Restoration Concept: ${idea.title}\n\nVision: ${idea.description}\n\nFocus on satisfying ASMR sounds of restoration: old to new, fresh painting, and high-gloss polish while keeping the original car model and intended color identical.`);
        setCarIdeas([]);
    };
    ...
    const handleGenerateScript = async () => {
        ...
        const voiceoverInstruction = noVoiceover ? "\n\nSTRICT RULE: Do NOT include any spoken dialogue or narration. Focus purely on visual actions and environmental ASMR sounds (scraping, sanding, spraying)." : "";
        const paintInstruction = repaintWhite ? "\n\nCRITICAL PAINT RULE: The final stages of restoration MUST result in a factory-perfect high-gloss WHITE paint finish (បាញ់ថ្នាំពណ៍សឡើងវិញ)." : "";
        ...
        1. Keep the exact same car model (make, year, shape) in every scene. Do NOT change the car type.
        2. Keep the exact same paint color throughout. If the car starts as a faded red classic, it must end as a high-gloss ruby red classic.
        3. Only change the state of the car (faded/messy to clean/shiny).
        4. Focus on PART REPLACEMENT: Show the old, rusty parts being physically removed and replaced with identical, brand-new, shiny chrome or painted parts.
        5. Maintain 100% environment and prop consistency (garage tools, lighting).
        `;

        try {
            const result = await generateConsistentStoryScript(
                `CAR RESTORATION ASMR SCRIPT. Context: ${masterPrompt}. Style: 100% Realistic, Professional Automotive Restoration Garage. ${consistencyInstruction}${voiceoverInstruction}${paintInstruction}`,
                sceneCount
            );
    ...
    const handleGenerateMetadata = async () => {
        ...
        setError(null);
        try {
            const context = `Car Restoration Project: ${masterPrompt}\n\nStoryboard:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(
                "Ultimate Car Restoration ASMR Transformation",
                context,
                "Car Restoration & Automotive ASMR"
            );
            setYoutubeMeta(meta);
        } catch (err) {
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const whiteConstraint = repaintWhite ? ", high gloss white paint finish" : "";
            // Strictly follow the same car model and color logic in the render prompt
            const prompt = `100% Realistic, professional car restoration photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Detailed textures, metal, subtle authentic mess, paint, tools. cinematic lighting${whiteConstraint}. CRITICAL: Keep the car model AND paint color exactly the same as in Scene 1. (សូមរក្សារ នៅម៉ូតឡានថ្នាំពណ៍ ដ៏ដែល). Show mechanical parts being swapped for new ones.`;
    ...
    const handleDownloadSenses = () => {
        ...
        link.href = url;
        link.download = `Car_Restoration_Senses_${Date.now()}.txt`;
    ...
    const handleDownloadJsonAll = () => {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `Car_Restoration_Storyboard_${Date.now()}.json`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        const link = document.createElement('a');
        link.href = url;
        link.download = `Car_Restoration_Sense_${num}.png`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleCopyJson = (index: number) => {
        ...
        const structuredData = {
            ...
            prompt: `100% Realistic, professional car restoration photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. (Model & Color Consistency: ON)`
        };
    ...
    const handleClear = () => {
        setMasterPrompt('Design Car (ASMR): Restoring a classic vintage car from old and rusty to 100% brand new. រថយន្តចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើង វិញ។ Focus on satisfying high-detail ASMR steps like washing, sanding, replacing broken parts with new ones, and repainting the original color to high-gloss perfection.');
        setScenes([]);
        setCarIdeas([]);
        setNoVoiceover(false);
        setRepaintWhite(false);
        setYoutubeMeta(null);
        setError(null);
        setSceneCount(15);
    };
                        ...
                        {localHistory.length > 0 ? (
                            localHistory.map((project, idx) => (
                                ...
                                >
                                    ...
                                    <p className="text-white text-xs font-bold truncate mb-1">Car Content</p>
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    <div className="flex justify-between items-start">
                        <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-600 mb-2 flex items-center gap-2">
                            <span>🚗</span> Design Car (ASMR)
                        </h2>
                        <div className="flex items-center gap-1.5 px-3 py-1 bg-emerald-900/40 border border-emerald-500/50 rounded-full text-emerald-400 shadow-lg">
                            <LockIcon />
                            <span className="text-[9px] font-black uppercase tracking-widest">Model & Color Locked</span>
                        </div>
                    </div>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest border-b border-gray-700 pb-4">
                        Restore legendary vehicles to factory perfection.
                    </p>
                    ...
                    <div className="space-y-4">
                        ...
                        {carIdeas.length > 0 && (
                            <div className="space-y-2 animate-slide-down bg-black/40 p-3 rounded-xl border border-gray-700 mb-4">
                                <h4 className="text-[10px] font-black text-orange-500 uppercase tracking-tighter mb-2">Select a Restoration Concept:</h4>
                        ...
                        <textarea 
                            ...
                            placeholder="Describe the car transformation..."
                        ...
                        />
                        <div className="p-2 bg-blue-900/30 border border-blue-500/30 rounded-lg text-[9px] text-blue-300 italic leading-tight flex items-start gap-2">
                            <LockIcon className="mt-0.5" />
                            <span>"សូមរក្សារ នៅម៉ូតឡានថ្នាំពណ៍ ដ៏ដែល គ្រាន់តែ ធ្វើផុសផុស ដោះដូ គ្រឿងបន្លាស់ នឹងបាញ់ថ្នាំ ពណ៍ឡើងវិញ" (Strict consistency active: Model and color locked).</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 space-y-4">
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative">
                                <input 
                                    type="checkbox" 
                                    checked={repaintWhite} 
                                    onChange={e => setRepaintWhite(e.target.checked)}
                                    className="sr-only"
                                />
                                <div className={`w-10 h-5 rounded-full transition-colors ${repaintWhite ? 'bg-orange-600' : 'bg-gray-700'}`}></div>
                                <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${repaintWhite ? 'translate-x-5' : 'translate-x-0'}`}></div>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-sm font-black text-orange-400 uppercase tracking-tighter">បាញ់ថ្នាំពណ៍សឡើងវិញ (Repaint White)</span>
                                <span className="text-[9px] text-gray-500 italic">Restore to factory white finish.</span>
                            </div>
                        </label>
                    </div>
