...
const LightningGenerator: React.FC = () => {
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            if (e.detail.tool !== 'the-lightning') return;
            const data = {
                id: crypto.randomUUID(),
                timestamp: Date.now(),
                tool: 'the-lightning',
                category: 'vip',
                title: "The Lightning Project",
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic 3D Pixar Disney Animation Style Render, 8k. Subject: The Lightning McQueen (red race car with expressive eyes on windshield). Action: ${scene.action}. Environment: ${scene.consistentContext}. Detailed textures (rust vs glossy paint), cinematic lighting. No text.`;
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in">
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500 mb-2 flex items-center gap-2">
                    ...
                    <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700">
                        <label className="flex items-center gap-3 cursor-pointer group">
                            ...
                            <div className="flex flex-col">
                                <span className="text-sm font-black text-red-400 uppercase tracking-tighter">No Voiceover</span>
                                <span className="text-[9px] text-gray-500 italic">Focus on satisfying ASMR racing sounds.</span>
                            </div>
                        </label>
                    </div>

                    <div>
                        <label className="block text-xs font-black text-gray-400 uppercase mb-2 tracking-widest">Number Senses (1-100)</label>
                        <input 
                            ...
                            {scenes.length > 0 && (
                                <>
                                    <button 
                                        ...
                                        className="flex items-center gap-2 px-3 py-2 bg-[#1e293b] border border-gray-700 hover:border-red-400 text-gray-300 rounded-lg text-xs font-bold transition shadow-md"
                                    >
                                        {copyStatus === 'all-senses-copy' ? '✓ Copied' : <><CopyIcon /> Copy All Senses</>}
                                    </button>
                                    <button 
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[85vh] overflow-y-auto custom-scrollbar pr-2 pb-20">
                                        ...
                                        <div className="flex flex-col items-center gap-3">
                                            {scene.isLoadingImage ? (
                                                <>
                                                    ...
                                                    <span className="text-[10px] text-gray-500 font-black uppercase animate-pulse">Rendering 3D Visual...</span>
                                                </>
                                            ) : (
                                                <button 
                                                    onClick={() => handleGenerateImage(idx)}
                                                    className="px-6 py-2 bg-gradient-to-r from-red-600 to-orange-600 text-white font-black rounded-full text-[10px] uppercase shadow-lg transition-all transform hover:scale-105"
                                                >
                                                    Render Sense Art
                                                </button>
                                            )}
                        ...
                        {scenes.length === 0 && !isGeneratingScript && (
                            <div className="col-span-full py-20 text-center bg-gray-900/20 rounded-[3rem] border-4 border-dashed border-gray-800 flex flex-col items-center justify-center">
                                 ...
                                 <p className="text-xl font-black text-gray-600 uppercase tracking-[0.4em]">Racing Floor Ready</p>
                                 <p className="text-sm text-gray-700 mt-4 max-w-md">Select a vision or describe your story and click "Get Sense Script" to begin.</p>
                            </div>
                        )}
                    </div>
                </div>
