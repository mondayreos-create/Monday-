...
// --- Audio Utilities ---
/* FIX: Implemented missing audioDecode utility. */
...
/* FIX: Implemented missing pcmToWav utility for adding WAV headers to raw PCM. */
...
const kiddePresets = [
    ...
    { title: "Wheels on the Bus Adventure | City Sightseeing 3D", synopsis: "The bright yellow school bus goes all through the town. Characters wave from the windows as the bus passes by a park, a bakery, and a library." },
...
const Kidde4DGenerator: React.FC = () => {
    ...
    const handleReloadHistory = (project: any) => {
        if (project.data.characterImages) setCharacterImages(project.data.characterImages);
        if (project.data.synopsis) setSynopsis(project.data.synopsis);
    ...
    const handleAutoSetup = async () => {
        ...
        try {
            ...
            const prompt = `Create ${count} distinct kid-friendly 3D characters or vehicles for this story: "${synopsis}". Describe them as cute 3D living characters with detailed textures.`;
    ...
    const handleCopySenseData = (index: number, mode: 'json' | 'prompt') => {
        ...
        if (mode === 'json') {
            ...
            handleCopyText(JSON.stringify(data, null, 2), `json-${index}`);
        } else {
            handleCopyText(scene.promptUsed || "", `prompt-${index}`);
        }
    };
    ...
    /* FIX: Defined missing handleDownloadImage function. */
                        ...
                        {localHistory.length > 0 ? (
                            localHistory.map((project, idx) => (
                                ...
                                >
                                    ...
                                    <p className="text-gray-400 text-xs line-clamp-2 italic leading-relaxed mb-3">"{project.data.synopsis}"</p>
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="bg-[#1e293b]/60 p-6 rounded-xl border border-gray-700 h-fit space-y-6 shadow-2xl">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                ...
                                {characterImages.length < 5 && (
                                    <label className="cursor-pointer aspect-square bg-gray-900 rounded-lg border-2 border-dashed border-gray-600 flex flex-col items-center justify-center hover:border-cyan-400 transition-colors">
                        ...
                        {isProcessing && <div className="w-full bg-gray-700 rounded-full h-2 mt-2 shadow-inner"><div className="bg-gradient-to-r from-cyan-400 to-pink-500 h-2 rounded-full transition-all duration-300 shadow-[0_0_10px_rgba(34,211,238,0.5)]" style={{ width: `${progress}%` }}></div></div>}
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-4 h-[85vh] overflow-y-auto custom-scrollbar pr-2 pb-20">
                    ...
                    {isProcessing && scenes.length === 0 && (
                        <div className="bg-[#0f172a] p-12 rounded-[2rem] border border-cyan-500/30 shadow-[0_0_80px_rgba(34,211,238,0.15)] flex flex-col items-center justify-center animate-fade-in min-h-[500px]">
                            <div className="relative w-48 h-48 mb-10">
                                 <svg className="w-full h-full transform -rotate-90">
                                    <circle cx="96" cy="96" r="88" stroke="currentColor" strokeWidth="14" fill="transparent" className="text-gray-800" />
                                    <circle cx="96" cy="96" r="88" stroke="currentColor" strokeWidth="14" fill="transparent" className="text-cyan-500 transition-all duration-700" strokeDasharray={2 * Math.PI * 88} strokeDashoffset={2 * Math.PI * 88 * (1 - progress / 100)} strokeLinecap="round" />
                                </svg>
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                    <span className="text-5xl font-black text-white drop-shadow-[0_0_15px_rgba(34,211,238,0.5)]">{progress}%</span>
                                </div>
                            </div>
                                ...
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            {scene.isLoading ? <><Spinner className="h-10 w-10 text-pink-500" /><span className="text-[10px] text-gray-500 font-black uppercase animate-pulse">Rendering 3D Art...</span></> : <span className="text-[10px] text-gray-700 font-black">READY</span>}
                                        </div>
                                    )}
