...
import { generateConsistentStoryScript, generateImage, generateBicycleRestorationIdeas, CarIdea, generateYouTubeMetadata, YouTubeMetadata } from '../services/geminiService.ts';
...
const DesignBicycleGenerator: React.FC = () => {
    const [masterPrompt, setMasterPrompt] = useState('Design Bicycle (ASMR): Restoring a classic vintage road bike from old and rusty to a custom-painted brand new masterpiece. ម៉ូតូចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើង វិញ។ Focus on satisfying high-detail ASMR steps: frame stripping, rust cleaning, chain greasing, and professional spray painting with a glossy finish.');
    const [sceneCount, setSceneCount] = useState(15);
    const [scenes, setScenes] = useState<Scene[]>([]);
    const [isGeneratingScript, setIsGeneratingScript] = useState(false);
    const [isGeneratingIdeas, setIsGeneratingIdeas] = useState(false);
    const [bikeIdeas, setBikeIdeas] = useState<CarIdea[]>([]);
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            if (e.detail.tool !== 'design-bicycle') return;
            const data = {
                id: crypto.randomUUID(),
                timestamp: Date.now(),
                tool: 'design-bicycle',
                category: 'vip',
                title: "Bicycle Restoration ASMR",
    ...
    const loadLocalHistory = () => {
        ...
        if (historyRaw) {
            try {
                ...
                const toolHistory = history.filter((p: any) => p.tool === 'design-bicycle');
    ...
    const handleGenerateIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateBicycleRestorationIdeas(masterPrompt || "Old to new bicycle restoration");
            setBikeIdeas(ideas);
        } catch (err) {
    ...
    const handleUseIdea = (idea: CarIdea) => {
        setMasterPrompt(`Bicycle Restoration Concept: ${idea.title}\n\nTransformation Vision: ${idea.description}\n\nFocus on the satisfying ASMR sounds of frame stripping, rust removal, engine-like cleaning of components, and high-quality repainting to make it look brand new. ម៉ូតូចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើង វិញ`);
        setBikeIdeas([]);
    };
    ...
    const handleGenerateScript = async () => {
        ...
        const voiceoverInstruction = noVoiceover ? "\n\nSTRICT RULE: Do NOT include any spoken dialogue or narration. Focus purely on environmental ASMR sounds: scraping, metal clinking, air puffing, and paint spraying." : "";

        try {
            const result = await generateConsistentStoryScript(
                `BICYCLE RESTORATION ASMR SCRIPT. Context: ${masterPrompt}. Style: 100% Realistic, Professional Workshop/Garage Macro Photography. Focus on the transformation from rusty junk to high-end bicycle art.${voiceoverInstruction}`,
                sceneCount
            );
    ...
    const handleGenerateMetadata = async () => {
        ...
        setError(null);
        try {
            const context = `Bicycle Transformation: ${masterPrompt}\n\nScenes:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(
                "Ultimate Satisfying Bicycle Restoration ASMR Transformation",
                context,
                "Bicycle Restoration & ASMR"
            );
            setYoutubeMeta(meta);
        } catch (err) {
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic, professional macro photography of bicycle restoration, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Detailed textures of rusty metal, polished chrome, fresh paint drops. Cinematic lighting.`;
    ...
    const handleDownloadSenses = () => {
        ...
        link.href = url;
        link.download = `Bicycle_Restoration_Senses_${Date.now()}.txt`;
    ...
    const handleDownloadJsonAll = () => {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `Bicycle_Restoration_Storyboard_${Date.now()}.json`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleCopyJson = (index: number) => {
        ...
        const data = {
            ...
            prompt: `100% Realistic, professional macro photography of bicycle restoration, 8k. Action: ${scene.action}. Context: ${scene.consistentContext}.`
        };
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        const link = document.createElement('a');
        link.href = url;
        link.download = `BicycleRestoration_Sense_${num}.png`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleClear = () => {
        setMasterPrompt('Design Bicycle (ASMR): Restoring a classic vintage road bike from old and rusty to a custom-painted brand new masterpiece. ម៉ូតូចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើង វិញ។ Focus on satisfying high-detail ASMR steps: frame stripping, rust cleaning, chain greasing, and professional spray painting with a glossy finish.');
        setScenes([]);
        setBikeIdeas([]);
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in">
                        ...
                        {localHistory.length > 0 ? (
                            localHistory.map((project, idx) => (
                                ...
                                >
                                    ...
                                    <p className="text-white text-xs font-bold truncate mb-1">Bicycle Content</p>
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-500 mb-2 flex items-center gap-2">
                        <span>🚲</span> Design Bicycle (ASMR)
                    </h2>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest border-b border-gray-700 pb-4">
                        Old Components to Masterpiece Finish.
                    </p>
                    ...
                    <div className="space-y-4">
                        ...
                        {bikeIdeas.length > 0 && (
                            <div className="space-y-2 animate-slide-down bg-black/40 p-3 rounded-xl border border-gray-700 mb-4">
                                <h4 className="text-[10px] font-black text-indigo-500 uppercase tracking-tighter mb-2">Select a Restoration Concept:</h4>
                                {bikeIdeas.map((idea, idx) => (
                        ...
                        <textarea 
                            ...
                            placeholder="Describe the bicycle restoration journey..."
                    ...
                    <button 
                        ...
                        className="w-full py-4 bg-gradient-to-r from-indigo-600 to-cyan-600 hover:brightness-110 text-white font-black rounded-xl shadow-xl transition transform active:scale-[0.98] disabled:opacity-50 text-lg flex items-center justify-center gap-3 uppercase tracking-widest"
                    >
