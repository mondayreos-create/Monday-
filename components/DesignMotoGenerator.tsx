...
const DesignMotoGenerator: React.FC = () => {
    const [masterPrompt, setMasterPrompt] = useState('Design Moto (ASMR): Restoring a classic vintage motorcycle from old and rusty to brand new. Focus on high-impact ASMR sounds like cleaning rust, engine repair, and fresh painting. ម៉ូតូចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើង វិញ។');
    const [sceneCount, setSceneCount] = useState(15);
    const [scenes, setScenes] = useState<Scene[]>([]);
    const [isGeneratingScript, setIsGeneratingScript] = useState(false);
    const [isGeneratingIdeas, setIsGeneratingIdeas] = useState(false);
    const [motoIdeas, setMotoIdeas] = useState<CarIdea[]>([]);
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            if (e.detail.tool !== 'design-moto') return;
            const data = {
                id: crypto.randomUUID(),
                timestamp: Date.now(),
                tool: 'design-moto',
                category: 'vip',
                title: "Motorcycle Restoration Project",
                data: { masterPrompt, sceneCount, scenes, motoIdeas, noVoiceover, youtubeMeta }
            };
        ...
        const handleLoad = (e: any) => {
            ...
            if (d.motoIdeas) setMotoIdeas(d.motoIdeas);
    ...
    }, [masterPrompt, sceneCount, scenes, motoIdeas, noVoiceover, youtubeMeta]);
    ...
    const loadLocalHistory = () => {
        ...
        if (historyRaw) {
            try {
                ...
                const toolHistory = history.filter((p: any) => p.tool === 'design-moto');
    ...
    const handleGenerateIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateCarRestorationIdeas("Motorcycle Restoration ASMR: " + masterPrompt);
            setMotoIdeas(ideas);
        } catch (err) {
            setError("Failed to generate moto ideas.");
        } finally {
    ...
    const handleUseIdea = (idea: CarIdea) => {
        setMasterPrompt(`Restoration Concept: ${idea.title}\n\nVision: ${idea.description}\n\nFocus on the satisfying ASMR sounds of a complete motorcycle restoration: taking apart the engine, cleaning rusty parts, and repainting the frame and tank to high-gloss perfection.`);
        setMotoIdeas([]);
    };
    ...
    const handleGenerateScript = async () => {
        ...
        const voiceoverInstruction = noVoiceover ? "\n\nSTRICT RULE: Do NOT include any spoken dialogue or narration. Focus purely on environmental ASMR sounds (clinking metal, scrubbing, spray painting)." : "";

        try {
            const result = await generateConsistentStoryScript(
                `MOTORCYCLE RESTORATION ASMR SCRIPT. Context: ${masterPrompt}. Style: 100% Realistic, Professional Motorcycle Garage Photography. Focus on the transformation from junk to jewel.${voiceoverInstruction}`,
                sceneCount
            );
    ...
    const handleGenerateMetadata = async () => {
        ...
        setError(null);
        try {
            const context = `Motorcycle Restoration Project: ${masterPrompt}\n\nStoryboard:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(
                "Satisfying Motorcycle Restoration ASMR",
                context,
                "Restoration & Garage ASMR"
            );
            setYoutubeMeta(meta);
        } catch (err) {
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic, professional automotive photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Detailed textures of chrome, grease, fresh paint, tools. cinematic natural garage lighting.`;
            ...
            setError("Image generation failed for sense " + scene.sceneNumber);
        }
    };
    ...
    const handleCopyJson = (index: number) => {
        ...
        const structuredData = {
            ...
            prompt: `100% Realistic, professional automotive photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Detailed textures of chrome, grease, fresh paint, tools. cinematic natural garage lighting.`
        };
    ...
    const handleDownloadSenses = () => {
        ...
        link.href = url;
        link.download = `Moto_Restoration_Senses_${Date.now()}.txt`;
    ...
    const handleDownloadJsonAll = () => {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `Moto_Restoration_Storyboard_${Date.now()}.json`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        const link = document.createElement('a');
        link.href = url;
        link.download = `MotoRestoration_Sense_${num}.png`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleClear = () => {
        setMasterPrompt('Design Moto (ASMR): Restoring a classic vintage motorcycle from old and rusty to brand new. Focus on high-impact ASMR sounds like cleaning rust, engine repair, and fresh painting. ម៉ូតូចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើង វិញ។');
        setScenes([]);
        setMotoIdeas([]);
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in">
                        ...
                        {localHistory.length > 0 ? (
                            localHistory.map((project, idx) => (
                                ...
                                >
                                    ...
                                    <p className="text-white text-xs font-bold truncate mb-1">Moto Content</p>
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-indigo-600 mb-2 flex items-center gap-2">
                        <span>🏍️</span> Design Moto
                    </h2>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest border-b border-gray-700 pb-4">
                        Restore legendary bikes to showroom quality.
                    </p>

                    <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            <label className="block text-xs font-black text-gray-400 uppercase tracking-widest">Master Prompt For Restoration</label>
                            <button 
                                onClick={handleGenerateIdeas}
                                disabled={isGeneratingIdeas}
                                className="text-[10px] font-black text-indigo-400 hover:text-indigo-300 transition-colors uppercase flex items-center gap-1"
                            >
                        ...
                        {motoIdeas.length > 0 && (
                            <div className="space-y-2 animate-slide-down bg-black/40 p-3 rounded-xl border border-gray-700 mb-4">
                                <h4 className="text-[10px] font-black text-indigo-500 uppercase tracking-tighter mb-2">Select a Viral Bike Project:</h4>
                                {motoIdeas.map((idea, idx) => (
                                    <div key={idx} className="bg-gray-800 p-2 rounded border border-gray-700 group">
                                        <div className="flex justify-between items-start gap-2">
                                            ...
                                            <button 
                                                ...
                                                className="px-2 py-1 bg-indigo-600 hover:bg-indigo-500 text-white text-[9px] font-black rounded uppercase shadow"
                                            >
                                                Use
                                            </button>
                                        </div>
                        ...
                        <textarea 
                            ...
                            placeholder="Describe the bike and restoration vision..."
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative">
                                <input 
                                    ...
                                    className="sr-only"
                                />
                                <div className={`w-10 h-5 rounded-full transition-colors ${noVoiceover ? 'bg-indigo-600' : 'bg-gray-700'}`}></div>
                            ...
                            <div className="flex flex-col">
                                <span className="text-sm font-black text-indigo-400 uppercase tracking-tighter">No Voiceover (ASMR Only)</span>
                                <span className="text-[9px] text-gray-500 italic">Purely focus on sound effects.</span>
                            </div>
                        </label>
                    </div>

                    <div>
                        <label className="block text-xs font-black text-gray-400 uppercase mb-2 tracking-widest">Number of Senses (Scenes)</label>
                        <input 
                            type="number" min="1" max="100" 
                            value={sceneCount}
                            onChange={(e) => setSceneCount(Math.max(1, Math.min(100, parseInt(e.target.value) || 1)))}
                            className="w-full bg-gray-900 border border-gray-600 rounded-xl p-3 text-white font-black text-center text-xl focus:ring-2 focus:ring-indigo-500 outline-none"
                        />
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full py-4 bg-gradient-to-r from-violet-600 to-indigo-600 hover:brightness-110 text-white font-black rounded-xl shadow-xl transition transform active:scale-[0.98] disabled:opacity-50 text-lg flex items-center justify-center gap-3 uppercase tracking-widest"
                    >
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-gray-800/80 p-5 rounded-2xl border border-gray-700 shadow-xl flex justify-between items-center sticky top-0 z-10 backdrop-blur gap-4">
                        <h3 className="text-xl font-black text-white uppercase tracking-tighter">Moto Transformation storyboard ({scenes.length} Senses)</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[85vh] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        {scenes.map((scene, idx) => (
                            <div key={idx} className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col group hover:border-indigo-500/50 transition-all duration-300">
                                        ...
                                        <div className="flex flex-col items-center gap-3">
                                            {scene.isLoadingImage ? (
                                                ...
                                                <button 
                                                    ...
                                                    className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-full text-[10px] uppercase shadow-lg transition-all"
                                                >
                                                    Render Sense Art
                                                </button>
                                            )}
                                ...
                                <div className="p-4 flex-grow flex flex-col bg-gradient-to-b from-gray-900 to-black">
                                    <p className="text-gray-300 text-xs leading-relaxed italic border-l-2 border-indigo-500 pl-3 mb-4 line-clamp-3 group-hover:line-clamp-none transition-all">
                                        "{scene.action}"
                                    </p>
                                    <div className="mt-auto grid grid-cols-2 gap-2 border-t border-gray-800 pt-3">
                                        ...
                                        <button 
                                            ...
                                            className="text-[10px] bg-gray-800 hover:bg-gray-700 text-indigo-400 hover:text-indigo-300 transition font-black uppercase flex items-center justify-center gap-1.5 py-1.5 rounded-lg border border-gray-700"
                                        >
