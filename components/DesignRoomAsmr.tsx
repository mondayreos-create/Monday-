...
const DesignRoomAsmr: React.FC = () => {
    const [masterPrompt, setMasterPrompt] = useState('Design Room (ASMR): Transform a very old, messy, and abandoned room into a modern luxury space. Focus on satisfying sound details: cleaning deep dust, scrubbing mold, scraping old paint, and high-gloss spray painting. Design Room ចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើងវិញ');
    ...
    const handleGenerateIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateRoomDesignIdeas(masterPrompt || "Old to New room transformation");
            setRoomIdeas(ideas);
        } catch (err) {
    ...
    const handleUseIdea = (idea: CarIdea) => {
        setMasterPrompt(`Room Transformation Concept: ${idea.title}\n\nTransformation Plan: ${idea.description}\n\nFocus on the satisfying ASMR sounds of a complete room restoration: cleaning the mess, fixing structural issues, and repainting everything to a fresh, themed, or brand-new finish. Design Room ចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើងវិញ`);
        setRoomIdeas([]);
    };
    ...
    const handleGenerateScript = async () => {
        ...
        const voiceInstruction = noVoiceover ? "\n\nSTRICT RULE: Do NOT include any spoken dialogue or narration. Focus purely on environmental ASMR sounds like scrubbing, vacuuming, painting, and construction clicks." : "";

        try {
            const result = await generateConsistentStoryScript(
                `ROOM RESTORATION ASMR PRODUCTION. Context: ${masterPrompt}. Style: 100% Realistic, Professional Interior Design/Renovation Photography. Show the satisfying progress from absolute ruin to modern or themed perfection.${voiceInstruction}`,
                sceneCount
            );
    ...
    const handleGenerateMetadata = async () => {
        ...
        setError(null);
        try {
            const context = `Room Restoration Project: ${masterPrompt}\n\nStoryboard:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(
                "Ultimate Satisfying Room Makeover ASMR",
                context,
                "Room Restoration & Renovation"
            );
            setYoutubeMeta(meta);
        } catch (err) {
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const styleContext = masterPrompt.toLowerCase().includes('mario') ? "Super Mario themed room, bright primary colors, brick patterns, cloud ceiling," : "";
            const prompt = `100% Realistic, professional interior photography, 8k. Action: ${scene.action}. ${styleContext} Setting: ${scene.consistentContext}. Detailed textures, dust particles, fresh paint wetness, cinematic lighting, wide angle.`;
    ...
    const handleCopyJson = (index: number) => {
        ...
        const structuredData = {
            ...
            prompt: `100% Realistic, high-end interior photography, 8k. Action: ${scene.action}. Context: ${scene.consistentContext}.`
        };
    ...
    const handleDownload = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `RoomMakeover_Sense_${num}.png`;
    ...
    const handleClear = () => {
        setMasterPrompt('Design Room (ASMR): Transform a very old, messy, and abandoned room into a modern luxury space. Focus on satisfying sound details: cleaning deep dust, scrubbing mold, scraping old paint, and high-gloss spray painting. Design Room ចាស់មក ផុសជុលបាញ់ថ្នាំពណ៍ មកថ្មីឡើងវិញ');
        setScenes([]);
        setRoomIdeas([]);
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in">
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-pink-600 mb-2 flex items-center gap-2">
                        <span>🛋️</span> Design Room (ASMR)
                    </h2>
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest border-b border-gray-700 pb-4">
                        Old ruin to fresh themed perfection.
                    </p>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <button 
                                ...
                                className="text-[10px] font-black text-fuchsia-400 hover:text-fuchsia-300 transition-colors uppercase flex items-center gap-1"
                            >
                        ...
                        {roomIdeas.length > 0 && (
                            <div className="space-y-2 animate-slide-down bg-black/40 p-3 rounded-xl border border-gray-700 mb-4">
                                <h4 className="text-[10px] font-black text-fuchsia-500 uppercase tracking-tighter mb-2">Select a Transformation Concept:</h4>
                                    ...
                                    <div key={idx} className="bg-gray-800 p-2 rounded border border-gray-700 group">
                                        <div className="flex justify-between items-start gap-2">
                                            ...
                                            <button 
                                                ...
                                                className="px-2 py-1 bg-fuchsia-600 hover:bg-fuchsia-500 text-white text-[9px] font-black rounded uppercase shadow"
                                            >
                                                Use
                                            </button>
                                        </div>
                        ...
                        <textarea 
                            ...
                            placeholder="Describe the room and makeover goals..."
                            className="w-full bg-gray-900 border border-gray-600 rounded-xl p-4 text-white h-48 resize-none focus:ring-2 focus:ring-fuchsia-500 outline-none shadow-inner text-sm leading-relaxed"
                        />
                    </div>
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative">
                                <input 
                                    ...
                                    className="sr-only"
                                />
                                <div className={`w-10 h-5 rounded-full transition-colors ${noVoiceover ? 'bg-fuchsia-600' : 'bg-gray-700'}`}></div>
                            ...
                            <div className="flex flex-col">
                                <span className="text-sm font-black text-fuchsia-400 uppercase tracking-tighter">No Voiceover (ASMR Only)</span>
                    ...
                    <button 
                        ...
                        className="w-full py-4 bg-gradient-to-r from-fuchsia-600 to-pink-600 hover:brightness-110 text-white font-black rounded-xl shadow-xl transition transform active:scale-[0.98] disabled:opacity-50 text-lg flex items-center justify-center gap-3 uppercase tracking-widest"
                    >
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-gray-800/50 p-5 rounded-2xl border border-gray-700 shadow-xl flex justify-between items-center sticky top-0 z-10 backdrop-blur">
                        <h3 className="text-xl font-black text-white uppercase tracking-tighter">Room transformation storyboard ({scenes.length} Senses)</h3>
                    ...
                    {isGeneratingScript && scenes.length === 0 && (
                        <div className="h-64 flex flex-col items-center justify-center bg-gray-900/50 rounded-2xl border border-dashed border-gray-700 animate-pulse">
                            <Spinner className="h-10 w-10 text-pink-500 mb-4" />
                            <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">Generating satisfyning transformation senses...</p>
                        </div>
                    )}

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[85vh] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        {scenes.map((scene, idx) => (
                            <div key={idx} className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col group hover:border-pink-500/50 transition-all duration-300">
                                        ...
                                        <div className="flex flex-col items-center gap-3">
                                            {scene.isLoadingImage ? (
                                                ...
                                                <button 
                                                    ...
                                                    className="px-6 py-2 bg-pink-600 hover:bg-pink-500 text-white font-black rounded-full text-[10px] uppercase shadow-lg transition-all"
                                                >
                                                    Render Sense Art
                                                </button>
                                            )}
                                ...
                                <div className="p-4 flex-grow flex flex-col bg-gradient-to-b from-gray-900 to-black">
                                    <p className="text-gray-300 text-xs leading-relaxed italic border-l-2 border-pink-500 pl-3 mb-4 line-clamp-3 group-hover:line-clamp-none transition-all">
                                        "{scene.action}"
                                    </p>
                                    <div className="mt-auto grid grid-cols-2 gap-2 border-t border-gray-800 pt-3">
                                        ...
                                        <button 
                                            ...
                                            className="text-[10px] bg-gray-800 hover:bg-gray-700 text-pink-400 hover:text-pink-300 transition font-black uppercase flex items-center justify-center gap-1.5 py-1.5 rounded-lg border border-gray-700"
                                        >
                        ...
                        {scenes.length === 0 && !isGeneratingScript && (
                            <div className="col-span-full py-20 text-center bg-gray-900/20 rounded-3xl border-2 border-dashed border-gray-800 flex flex-col items-center justify-center">
                                 ...
                                 <p className="text-[10px] text-gray-700 mt-2">Enter your Master Prompt and click "Get Sense Script" to start.</p>
                            </div>
                        )}
                    </div>
                </div>
