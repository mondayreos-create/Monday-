...
import { assistWriting, generateVoiceover, generateImageForScene, extendStoryWithImage, generateYouTubeMetadata, YouTubeMetadata, generateCharacters } from '../services/geminiService.ts';
                    ...
                    <div className="bg-gray-800/60 p-4 sm:p-6 rounded-2xl border border-gray-700 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                            ...
                            <div className="flex gap-2 w-full sm:w-auto">
                                {metadata && (
                                     <button 
                                        onClick={() => handleCopyMeta(JSON.stringify(metadata, null, 2), 'full-meta')} 
                        ...
                        {metadata ? (
                            <div className="space-y-4">
                                <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700 shadow-inner">
                                    <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest"><span>POST TITLE / VIDEO TITLE</span><button onClick={() => handleCopyMeta(metadata.title, 'title')} className="hover:text-white transition-colors">{copiedMeta === 'title' ? '✓ COPIED' : 'COPY'}</button></div>
                                    <div className="text-white text-sm font-black uppercase tracking-tight">{metadata.title}</div>
                                </div>
                                <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700 shadow-inner">
                                    <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest"><span>DESCRIPTION FOR YOUTUBE</span><button onClick={() => handleCopyMeta(metadata.description, 'desc')} className="hover:text-white transition-colors">{copiedMeta === 'desc' ? '✓ COPIED' : 'COPY'}</button></div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700 shadow-inner">
                                        <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest"><span>#HASHTAGS</span><button onClick={() => handleCopyMeta(metadata.hashtags.join(' '), 'hashtags')} className="hover:text-white transition-colors">{copiedMeta === 'hashtags' ? '✓ COPIED' : 'COPY'}</button></div>
                                        <div className="text-blue-400 text-xs font-bold font-mono">{metadata.hashtags.join(' ')}</div>
                                    </div>
                                    <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700 shadow-inner">
                                        <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest"><span>KEYWORDS RANGING</span><button onClick={() => handleCopyMeta(metadata.keywords.join(', '), 'keywords')} className="hover:text-white transition-colors">{copiedMeta === 'keywords' ? '✓ COPIED' : 'COPY'}</button></div>
                                        <div className="text-cyan-400 text-xs font-medium font-mono">{metadata.keywords.join(', ')}</div>
                                    </div>
                                </div>
                            </div>
                        ) : (
